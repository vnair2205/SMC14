...
exports.refineTopic = async (req, res) => {
    ...
    try {
        ...
        // 1. Conditionally create the main instruction for the AI.
        const titleInstruction = (languageName === 'English')
            // If English, give a simple, clear instruction.
            ? `Each object should have "title" and "englishTitle" keys, both containing the same SEO-friendly course title in English.`
            // For other languages, keep the original detailed instruction.
            : `Each object should have a "title" in the ${languageName} (${nativeName}) language and an "englishTitle" in English.`;

        // 2. Build the final prompt with the corrected instruction.
        const prompt = `A user wants to learn about: "${topic}". Suggest three improved course titles. 
        IMPORTANT: Your response MUST be a valid JSON array of objects. ${titleInstruction}
        Example: [{"title": "Title 1", "englishTitle": "English Title 1"}, {"title": "Title 2", "englishTitle": "English Title 2"}, {"title": "Title 3", "englishTitle": "English Title 3"}]`;
...
exports.generateIndex = async (req, res) => {
    ...
    if (req.body.isPreGenerated) {
        return res.status(400).json({ msg: 'This action is not applicable to pre-generated courses.' });
    }
...
exports.generateLessonContent = async (req, res) => {
    ...
    try {
        ...
        if (shouldGenerateNewContent) {
            ...
            try {
             ...
             const searchQuery = encodeURIComponent(`${englishLessonTitle} ${englishSubtopicTitle} tutorial explanation`);
             const maxResults = 15; // Increased from 10
            const youtubeApiUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${searchQuery}&type=video&videoEmbeddable=true&maxResults=${maxResults}&key=${apiKey}&relevanceLanguage=en&videoDuration=medium`;
                
                console.log(`[YouTube] Searching with refined query (initial generation): "${decodeURIComponent(searchQuery)}"...`);
    ...
    // --- FIX: Log the full error stack trace ---
...
exports.changeLessonVideo = async (req, res) => {
    console.log('[API] /lesson/change-video CONTROLLER REACHED'); // Keep this log
    const { courseId, subtopicId, lessonId } = req.body;
    const userId = req.user.id;
    try {
        const course = await Course.findOne({ _id: courseId, user: userId });
        ...
        // --- FIX 1: Refined Search Query ---
        // Prioritize lesson and subtopic title, keep it concise
        const searchQuery = encodeURIComponent(`${englishLessonTitle} ${englishSubtopicTitle} tutorial explanation`); 
        
        // --- FIX 2: Increase maxResults ---
        const maxResults = 15; // Increased from 10

        const youtubeApiUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${searchQuery}&type=video&videoEmbeddable=true&maxResults=${maxResults}&key=${apiKey}&relevanceLanguage=en&videoDuration=medium`;

        console.log(`[YouTube] Searching for new video with refined query: "${decodeURIComponent(searchQuery)}"...`);
        const youtubeResponse = await axios.get(youtubeApiUrl);
        
        // Keep existing logic to filter out already seen videos
        ...
        console.log(`[YouTube] Found ${youtubeResponse.data.items.length} total results, ${newVideos.length} are new candidates.`);

        // Use the existing findBestVideo function
        ...
        if (bestVideo) {
            // Keep existing logic to save the new video
            const newVideoUrl = `https://www.youtube.com/embed/${bestVideo.id.videoId}`;
            ...
            // Ensure videoHistory exists before pushing
            if (!lesson.videoHistory) {
                 lesson.videoHistory = [];
            }
            lesson.videoHistory.push({ 
                 videoUrl: newVideoUrl, // Store the embed URL consistently
                 videoChannelId: lesson.videoChannelId, 
                 videoChannelTitle: bestVideo.snippet.channelTitle 
            });
            
            await course.save();
            console.log(`[YouTube] Found new best video ID: ${bestVideo.id.videoId}. Change count: ${lesson.videoChangeCount}`);
            res.json(lesson); // Send back the updated lesson data
        } else {
            // This is where the 404 originates
            console.warn('[YouTube] Could not find a new, unplayed video meeting relevance criteria.'); 
            return res.status(404).json({ msgKey: 'errors.no_new_video_found' }); // Return 404 as before
        }
    } catch (error) {
        // --- IMPROVED LOGGING ---
        console.error("--- ERROR IN changeLessonVideo ---", error.response ? JSON.stringify(error.response.data, null, 2) : error.message);
        // Log the stack trace if it's not an Axios error with a response
        if (!error.response) {
...
exports.exportCourseAsPdf = async (req, res) => {
    ...
    headless: true, 
    // --- START FIX ---
    // Specify the path where Render likely installs Chromium
    executablePath: process.env.CHROMIUM_PATH || '/usr/bin/google-chrome-stable', // Common paths, adjust if Render docs specify differently
    args: [
        '--no-sandbox', 
        '--disable-setuid-sandbox',
        // Add other potentially necessary flags for Linux environments:
        '--disable-dev-shm-usage', // Prevent issues with limited shared memory
...
exports.startPreGeneratedCourse = async (req, res) => {
    try {
        const preGenCourseId = req.params.id;
        const userId = req.user.id;

        // 1. Check if the user has already started this course
        const existingCourse = await Course.findOne({ user: userId, preGeneratedCourseId: preGenCourseId });
        if (existingCourse) {
            return res.status(409).json({ msgKey: 'errors.courseAlreadyStarted', courseId: existingCourse._id });
        }

        // 2. Find the pre-generated course template
        const preGeneratedCourse = await PreGenCourse.findById(preGenCourseId);
        if (!preGeneratedCourse) {
            return res.status(404).json({ msgKey: 'errors.courseNotFound' });
        }

        // 3. Create a new course for the user based on the template
        const newCourse = new Course({
            user: userId,
            topic: preGeneratedCourse.topic,
            objective: preGeneratedCourse.objective,
            outcome: preGeneratedCourse.outcome,
            index: preGeneratedCourse.index,
            language: preGeneratedCourse.language,
            thumbnailUrl: preGeneratedCourse.thumbnailUrl,
            isPreGenerated: true,
            preGeneratedCourseId: preGenCourseId
        });
        ...
        console.error("Error starting pre-generated course:", error.message);
        res.status(500).json({ msgKey: "errors.generic" });
    }
};