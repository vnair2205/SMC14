...
exports.startCourse = async (req, res) => {
    try {
        ...
        if (existingCourse) {
            return res.status(200).json({ courseId: existingCourse._id }); // Not an error, just redirect
        }

        // 2. Find the template course to copy
        ...
        // --- FIX for 100% Completion Bug ---
        // Manually create a new index with all lessons marked as not completed.
        const resetIndex = {
            subtopics: templateCourse.index.subtopics.map(subtopic => ({
                ...subtopic,
                lessons: subtopic.lessons.map(lesson => ({
                    ...lesson,
                    isCompleted: false // Explicitly set completion to false for the user
                }))
            }))
        };
        // --- End of Fix ---

        // 3. Create the new course for the user
        const newCourse = new Course({
            ...
            // Apply the reset index
            index: resetIndex,

            // Set user-specific and tracking fields
            user: userId,
            status: 'Active',
            isPreGenerated: true, // --- FIX for Quota Bug: Clearly mark this course type
        ...
        console.error("Error starting pre-generated course:", error);
