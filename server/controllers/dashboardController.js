...
exports.getWelcomeData = async (req, res) => {
    try {
        ...
        if (user.activeSubscription) {
            ...
            // Check if the subscription and its populated plan are valid
            if (subscription && subscription.plan) {

                // --- FIX: Use the correct field names from your database schema ---
                const totalQuota = subscription.plan.coursesPerMonth || 0;
                const renewalDate = subscription.endDate; // Use endDate, not renewalDate
                // --- End of Fix ---

                const coursesGeneratedThisPeriod = await Course.countDocuments({
                    user: req.user.id,
                    isPreGenerated: { $ne: true },
                    createdAt: { $gte: subscription.startDate, $lte: renewalDate }
                });

                const remainingQuota = Math.max(0, totalQuota - coursesGeneratedThisPeriod);

                responseData = {
                    ...responseData,
                    planName: subscription.plan.name,
                    renewalDate: renewalDate, // Use the corrected variable
                    courseQuota: `${remainingQuota} / ${totalQuota}`
                };
            }
        }
        ...
        console.error("Error in getWelcomeData:", err.message);
        res.status(500).send('Server Error');
    }
};
