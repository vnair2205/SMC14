...
const getSessionDetails = (req) => {
    ...
    // --- FIX 1: Clean up IP Address for Geolocation Lookup ---
    let lookupIp = ip;
    
    // Check if the server is configured to trust proxies (e.g., in Render)
    // If not, we might still see ::ffff:10.x.x.x (private IP) or the proxy IP.
    if (ip && ip.startsWith('::ffff:')) {
        lookupIp = ip.substring(7); // Removes '::ffff:' prefix if present
    }
    
    // --- FIX 2: Enhance Location Formatting ---
    const geo = geoip.lookup(lookupIp);
    
    let location = 'Unknown Location';
    
    if (geo && geo.country && geo.country !== 'RD') { // RD is a geoip-lite code for private IPs
        const city = geo.city || '';
        const region = geo.region || ''; // This is the state/region code (e.g., 'MH' for Maharashtra)
        const country = geo.country || '';

        // Format: City, State/Region (Country Code)
        if (city && region) {
    ...
    return { 
        ipAddress: lookupIp, // Return the cleaned IP
        device: `${agent.toAgent()} on ${agent.os.toString()}`, 
        location,
        loggedInAt: new Date(), // Add timestamp here for display
    };
};
...
exports.verifyPhoneOtp = async (req, res) => {
    ...
    try {
        ...
        if (!user || !user.pendingNewPhone) {
            console.log(`[Verify Phone Update] Failure: User ${req.user.id} not found or no pending phone update.`);
            return res.status(400).json({ msg: 'No phone number update pending for this user or user not found.' });
        }

        const phoneNumberToVerify = user.pendingNewPhone;
        console.log(`[Verify Phone Update] Attempting to verify OTP ${otp} for number ${phoneNumberToVerify} with Twilio.`);

        // --- Use Twilio to check the verification code ---
        const verification_check = await client.verify.v2.services(messagingServiceSid)
           .verificationChecks
           .create({ to: phoneNumberToVerify, code: otp });

        if (verification_check.status === 'approved') {
            console.log(`[Verify Phone Update] Twilio verification successful for ${phoneNumberToVerify}.`);
            // Update the user's phone number
            user.phoneNumber = phoneNumberToVerify;
            user.isPhoneVerified = true; // Ensure this is set if it wasn't already
            // Clear the temporary fields
            user.pendingNewPhone = undefined;
            user.phoneOtp = undefined; // Clear old fields just in case
            user.phoneOtpExpires = undefined; // Clear old fields just in case

            await user.save();
            console.log(`[Verify Phone Update] User ${user.email}'s phone number updated successfully.`);
            res.json({ msg: 'Phone number updated successfully.' });

        } else {
            // Twilio rejected the OTP
            console.log(`[Verify Phone Update] Twilio verification failed for ${phoneNumberToVerify}. Status: ${verification_check.status}`);
            res.status(400).json({ msg: 'Invalid or expired OTP.' });
        }

    } catch (err) {
        console.error('[Verify Phone Update] Error:', err);
        // Handle specific Twilio errors like 'too many attempts' if needed
        if (err.code === 20404) { // Not found - might mean the verification expired on Twilio's side
             return res.status(400).json({ msg: 'Verification failed. The OTP may have expired or the phone number is incorrect.' });
        }
...
exports.updateAndResendPhoneOtp = async (req, res) => {
    ...
    try {
        ...
        const existingUser = await User.findOne({ phoneNumber: newPhoneNumber, isPhoneVerified: true }); // Added isPhoneVerified check
        if (existingUser) {
            // Return a specific error if the number is already taken and verified
            return res.status(400).json({ msg: 'Phone number is already registered and verified by another user.' });
        }
        ...
        if(user.phoneNumber === newPhoneNumber) {
             return res.status(400).json({ msg: 'This is already your current phone number.' });
        }

        // Set the pending number
        user.pendingNewPhone = newPhoneNumber;
        // Clear any old phone OTP data just in case
        user.phoneOtp = undefined;
        user.phoneOtpExpires = undefined;
        await user.save();
        ...
        if (!otpResult.success) {
            // If Twilio fails, return Twilio's error or a generic one
            return res.status(500).json({ msg: otpResult.error || 'Failed to send OTP to the new phone number.' });
        }

        res.json({ msg: `OTP sent successfully to ${newPhoneNumber}. Please verify.` }); // More informative message

    } catch (err) {
        console.error('Error in updateAndResendPhoneOtp:', err); // Log the specific error
        // Check for specific Mongoose errors if needed, otherwise send generic
        if (err.name === 'ValidationError') {
             return res.status(400).json({ msg: 'Invalid data provided.', details: err.errors });
        }
        res.status(500).send('Server error occurred while updating phone number.'); // More specific server error
    }
};
