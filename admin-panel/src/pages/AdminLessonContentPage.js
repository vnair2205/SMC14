    ...
    useEffect(() => {
        const fetchChatHistory = async () => {
            ...
            try {
                ...
                // *** FIX 1: Retrieve the userId (creator) from the course object ***
                // The backend route is: /api/admin/users/:userId/courses/:courseId/chat
                // We assume the user ID is stored in a field like 'user' or 'userId' on the course object.
                const userId = course?.user || course?.userId;
                
                if (!userId) {
                    // This case should ideally not happen if the course object is complete
                    console.warn("User ID not found in course context. Cannot fetch chat history.");
                    setLoadingChat(false);
                    return;
                }

                // *** FIX 2: Correct the API URL to match the backend route structure ***
                const res = await axios.get(`/api/admin/users/${userId}/courses/${courseId}/chat`, {
        ...
        // *** FIX 3: Add 'course' to the dependency array, since we rely on it for userId ***
        if (courseId && course) fetchChatHistory();
    }, [courseId, course]);
    ...
    return (
        <>
            ...
            <Chatbot
                ...
                isReadOnly={true}
                // *** FIX 4: Use $isOpen to prevent the styled-components/React warning ***
                $isOpen={isChatOpen} 
