...
const ModalContent = styled.div`
  ...
  text-align: center;
`;
// ... (rest of styled components: Input, ButtonGroup, Button, StatusMessage, ResendText, SuccessIcon) ...
...
const UpdateEmailModal = ({ isOpen, onClose, currentUserEmail, onUpdateSuccess }) => {
    ...
    // --- FIX 2: No need for manual token ---
    ...
    const handleSendOtp = async () => {
        ...
        setStatus({ message: '', type: '' });
        try {
            // --- FIX 3: Use 'api.post', remove '/api' prefix, remove headers ---
            await api.post('/auth/update-email',
                { oldEmail: currentUserEmail, newEmail }
                // No headers needed, 'api.js' handles the token
            );
            setStep('enterOtp');
            setTimer(60);
        } catch (err) {
    ...
    const handleVerifyOtp = async () => {
        ...
        setStatus({ message: '', type: '' });
        try {
            // --- FIX 4: Use 'api.post', remove '/api' prefix, remove headers ---
