...
`;
// ... (rest of the styled components: ChangePasswordButton, IndianStatesDropdown, PersonalInfoContainer, ProfileHeader, AvatarWrapper, ProfileImage, ProfileInitials, CameraIconOverlay, UserName, Section, SectionTitle, InfoRow, UpdateButton, FormGroup, Label, Input, SaveButton, AddressDisplay, spin, SectionTitleHeader, LoaderOverlay) ...
...
const PersonalInfo = ({ user, onUpdate }) => {
    ...
    const handleSaveProfilePic = async (image, crop) => {
        ...
        try {
            // --- FIX 2: Use 'api.post' and remove URL prefix & token ---
            await api.post('/profile/picture', { imageUrl: dataUrl });
            onUpdate();
        } catch (error) {
   ...
   const handleSave = async () => {
        ...
        try {
            ...
            // --- FIX 3: Use 'api.put' and remove URL prefix & token ---
            await api.put('/profile/personal', payload);
            onUpdate();
        } catch (error) {
    ...
    const handleSaveAddress = async (newAddress) => {
        try {
            ...
            // --- FIX 4: Use 'api.put', fix URL path, remove URL prefix & token ---
            await api.put('/profile/personal', payload); // Use the correct profile route
            onUpdate();
