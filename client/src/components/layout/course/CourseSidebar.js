...
const CourseSidebar = ({ isRTL, course, isOpen, setIsOpen }) => {
    ...
    const handleExportPdf = async () => {
        ...
        try {
            // --- FIX: Use api.get, remove /api prefix, remove headers/token, add responseType ---
            const res = await api.get(`/course/export/${course._id}`, {
            ...
            // Attempt to read error message from blob if it's a JSON error response
            if (err.response && err.response.data instanceof Blob && err.response.data.type === "application/json") {
                const reader = new FileReader();
                reader.onload = () => {
                    try {
                        const errorData = JSON.parse(reader.result);
                        setErrorModal({ isOpen: true, message: t(errorData.msgKey || 'errors.generic') });
                    } catch (parseError) {
                        setErrorModal({ isOpen: true, message: t('errors.generic') });
                    }
                };
                reader.onerror = () => {
                    setErrorModal({ isOpen: true, message: t('errors.generic') });
