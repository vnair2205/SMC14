...
const Chatbot = ({ course, courseCompletedAndPassed, isRTL, fixedSideOffset = '20px', lessonContent }) => {
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
        if (isModalOpen && course?._id) {
            try {
                // --- FIX: Use api.get and remove /api prefix & config ---
                const res = await api.get(`/course/chat/${course._id}`);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
        ...
        const chatHistoryForApi = messages.slice(-10); 
        const body = { 
            courseId: course._id, 
        ...
        const res = await api.post('/course/chatbot', body);
