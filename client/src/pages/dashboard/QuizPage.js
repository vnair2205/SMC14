// client/src/pages/dashboard/QuizPage.js
...
const QuizPage = () => {
    ...
    useEffect(() => {
        const fetchQuiz = async () => {
            ...
            try {
                // --- FIX: Use api.post and remove /api prefix & config ---
                const res = await api.post('/course/quiz/generate', { courseId }); // <-- Corrected Call
                setQuiz(res.data);
            } catch (err) {
    ...
    // Add a check in case quiz data is empty or invalid after loading
    if (!quiz || quiz.length === 0) {
         navigate(`/course/${courseId}`); // Navigate back if quiz isn't loaded
    ...
    // Add a safeguard in case currentQuestion is somehow undefined
    if (!currentQuestion) {
        console.error("Error: currentQuestion is undefined at index", currentQuestionIndex);
        navigate(`/course/${courseId}`); // Navigate back as a safety measure
        return null;
    }
    ...
    return (
        <QuizOverlay>
            <QuizContent>
                ...
                <OptionsContainer>
                    {/* Ensure currentQuestion.options exists before mapping */}
