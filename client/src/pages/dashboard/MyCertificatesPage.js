...
// --- FIX 1: ADD THE REUSABLE DEBOUNCE HOOK ---
...
const MyCertificatesPage = () => {
  ...
  const fetchCertificates = useCallback(async (searchQuery) => {
    ...
    const config = { 
        ...
        params: {
            certified: true,
            limit: 100, // Or implement pagination if needed
            search: searchQuery,
        },
    };

    try {
        const certResponse = await axios.get('/api/course/', config);
  ...
  // --- FIX 3: UPDATE THIS EFFECT TO DEPEND ON THE DEBOUNCED TERM ---
  useEffect(() => {
    // Only fetch certificates if we have the user data.
    // The effect will now re-run ONLY when debouncedSearchTerm changes.
    if (user) { 
