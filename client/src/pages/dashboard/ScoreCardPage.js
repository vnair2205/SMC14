// client/src/pages/dashboard/ScoreCardPage.js
...
import { FiAward, FiHome, FiArrowRight, FiThumbsUp, FiRepeat } from 'react-icons/fi';
...
const ScoreCardPage = () => {
    ...
    const updateQuizCompletion = async () => {
        // --- FIX: Removed manual token check ---
        console.log(`[ScoreCardPage] Sending score to server: score=${score}, total=${total}, courseId=${courseId}`);
        try {
            // --- FIX: Use api.put, remove /api prefix & config ---
            await api.put(
                `/course/complete-quiz/${courseId}`, // Corrected URL
                { score: score, totalQuestions: total } // Body
            ); // Removed config
    ...
    useEffect(() => {
        if (score !== undefined && total !== undefined && courseId) { // Added courseId check
            updateQuizCompletion();
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [score, total, courseId]); // Keep dependencies
    ...
    const handleBackToCoursePage = () => {
        // Navigate back to the main course view page after quiz completion
        ...
        <ScorePageContainer>
            <ScoreCard passed={hasPassed}>
                ...
                <ScoreText>
                    ...
                    <span>{score} / {total}</span> ({percentage}%)
                </ScoreText>
