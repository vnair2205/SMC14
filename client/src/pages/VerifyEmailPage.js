...
        try {
            // 'response' holds the object from the successful API call
            const response = await api.post('/auth/verify-signup-email', {
                email: currentEmail,
                otp: otp
            });
            
            // --- FIX 1: Use 'response.data.token' instead of 'res.data.token' ---
...
        } catch (err) {
            // --- FIX 2: Use response.data.msgKey or fall back to a generic translation ---
            const errorKey = err.response?.data?.msg || 'errors.generic_verification_failed';
            // The console error about missing key means 'Verification failed. Please try again.' is not a valid key.
            // Using the received message if it's not a key, or falling back to a known key.
            const errorMsg = t(errorKey) !== errorKey ? t(errorKey) : errorKey;
            
            setNotification({ type: 'error', text: errorMsg });
            localStorage.removeItem('token'); // Clear token in case it was set improperly elsewhere
            setIsLoading(false);
        }
    };
